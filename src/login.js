...
module.exports = async (browser, email, password, puppeteerAuthenticate, gmailVerificationCodeParser) => {
  ...
  return page.waitFor('input[role=combobox]', {
    ...
    .catch(async () => {
      ...
      const email_verification_input = await page.$("#input__email_verification_pin")

      if (email_verification_input) {
        const verification_submit = await page.$("#email-pin-submit-button")
        ...
        if (verification_submit) {
          if (gmailVerificationCodeParser) {
            ...
            await verification_submit.click()
            ...
            return Promise.reject(new Error(`Verification pin requested but 'gmailVerificationCodeParser' was ${gmailVerificationCodeParser}`))
          }
        } else {
          logger.warn("Found verification pin input but couldn't find submit button")
        }
      }
